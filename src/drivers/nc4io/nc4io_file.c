    ...
    MPI_Comm comm, const char *path, int cmode, int ncid, MPI_Info info, void **ncpp) /* OUT */
{
    ...
    err = nc4ioi_req_list_init (&(nc4p->wreqs));
    if (err != NC_NOERR) return err;
    err = nc4ioi_req_list_init (&(nc4p->rreqs));
    if (err != NC_NOERR) return err;
...
int nc4io_open (MPI_Comm comm, const char *path, int omode, int ncid, MPI_Info info, void **ncpp) {
    char *filename;
    int err, ncidtmp;
    NC_nc4 *nc4p;
...
int nc4io_close (void *ncdp) {
    ...
    if (nc4p->mpiinfo != MPI_INFO_NULL) MPI_Info_free (&nc4p->mpiinfo);

    NCI_Free (nc4p->path);

    err = nc4ioi_req_list_free (&(nc4p->wreqs));
    if (err != NC_NOERR) return err;
    err = nc4ioi_req_list_free (&(nc4p->rreqs));
    if (err != NC_NOERR) return err;
...
int nc4io__enddef (void *ncdp,
    ...
    /* Call nc__enddef */
    err = nc__enddef (nc4p->ncid, (size_t)h_minfree, (size_t)v_align, (size_t)v_minfree,
                      (size_t)r_align);
    if (err != NC_NOERR) DEBUG_RETURN_ERROR (err);
...
int nc4io_begin_indep_data (void *ncdp) {
    int i, err, nvar;
    NC_nc4 *nc4p = (NC_nc4 *)ncdp;

    /* Make sure we are in data mode */
    if (fIsSet (nc4p->flag, NC_MODE_DEF)) DEBUG_RETURN_ERROR (NC_EINDEFINE);
...
int nc4io_end_indep_data (void *ncdp) {
    int i, err, nvar;
    NC_nc4 *nc4p = (NC_nc4 *)ncdp;
...
int nc4io_inq (void *ncdp, int *ndimsp, int *nvarsp, int *nattsp, int *xtendimp) {
    int err;
    NC_nc4 *nc4p = (NC_nc4 *)ncdp;
...
int nc4io_inq_misc (void *ncdp,
    ...
    if (num_fix_varsp != NULL || num_rec_varsp != NULL || recsize != NULL) {
        ...
        /* Set up the flag */
        for (i = 0; i < nrecdim; i++) { isrec[recdims[i]] = 1; }
    ...
    if (put_size != NULL) { *put_size = nc4p->putsize; }

    /* Get size counted by the driver */
    if (get_size != NULL) { *get_size = nc4p->getsize; }
...
int nc4io_cancel (void *ncdp, int num_req, int *req_ids, int *statuses) {
    /* We do not support nonblocking I/O yet */
    DEBUG_RETURN_ERROR (NC_ENOTSUPPORT);
}

int nc4io_wait (void *ncdp, int num_reqs, int *req_ids, int *statuses, int reqMode) {
    ...
    if (num_reqs >= 0) {
        ...
        for (i = 0; i < num_reqs; i++) {
            ...
            if (req_ids[i] & 1) {
                while (idx >= 0) {
                    idx = nc4p->rreqs.items[idx].next;
                    nr++;
                }
            } else {
                while (idx >= 0) {
                    idx = nc4p->wreqs.items[idx].next;
                    nw++;
                }
            }
        }
    ...
    wreqs = (nc4_req *)NCI_Malloc (sizeof (nc4_req) * nw);
    if (!wreqs) {
        DEBUG_ASSIGN_ERROR (status, NC_ENOMEM)
        goto fn_exit;
    }
    rreqs = (nc4_req *)NCI_Malloc (sizeof (nc4_req) * nr);
    if (!rreqs) {
        DEBUG_ASSIGN_ERROR (status, NC_ENOMEM)
        goto fn_exit;
    }
    if (num_reqs >= 0) {
        ...
        for (i = 0; i < num_reqs; i++) {
            ...
            if (req_ids[i] & 1) {
                while (idx >= 0) {
                    rreqs[nr] = nc4p->rreqs.items[idx];
                    err       = nc4ioi_req_list_remove (&(nc4p->rreqs), idx);
                    if (err != NC_NOERR) {
                        DEBUG_ASSIGN_ERROR (statuses[i], NC_EINVAL)
                        continue;
                    }
                    rreqs[nr].idp = i;
                ...
                while (idx >= 0) {
                    wreqs[nw] = nc4p->wreqs.items[idx];
                    err       = nc4ioi_req_list_remove (&(nc4p->wreqs), idx);
                    if (err != NC_NOERR) {
                        DEBUG_ASSIGN_ERROR (statuses[i], NC_EINVAL)
                        continue;
                    }
        ...
        if (num_reqs == NC_REQ_ALL || num_reqs == NC_PUT_REQ_ALL) {
            nw = 0;
            while (nc4p->wreqs.nused) {
                idx       = nc4p->wreqs.idx[0];
                wreqs[nw] = nc4p->wreqs.items[idx];
                nc4ioi_req_list_remove (&(nc4p->wreqs), idx);
        ...
        if (num_reqs == NC_REQ_ALL || num_reqs == NC_GET_REQ_ALL) {
            nr = 0;
            while (nc4p->rreqs.nused) {
                idx       = nc4p->rreqs.idx[0];
                rreqs[nr] = nc4p->rreqs.items[idx];

                nc4ioi_req_list_remove (&(nc4p->rreqs), idx);
                rreqs[nr++].idp = -1;
            }
        }
    }

    err = nc4ioi_flush_write_reqs (nc4p, nw, wreqs, statuses, reqMode & NC_REQ_COLL);
    if (status == 0) status = err;

    err = nc4ioi_flush_read_reqs (nc4p, nr, rreqs, statuses, reqMode & NC_REQ_COLL);
    if (status == 0) status = err;
...
int nc4io_set_fill (void *ncdp, int fill_mode, int *old_fill_mode) {
    int err;
    NC_nc4 *nc4p = (NC_nc4 *)ncdp;
...
int nc4io_fill_var_rec (void *ncdp, int varid, MPI_Offset recno) {
    /* NetCDF does not support this natively */
    DEBUG_RETURN_ERROR (NC_ENOTSUPPORT)
}

int nc4io_def_var_fill (void *ncdp, int varid, int no_fill, const void *fill_value) {
    int err;
    NC_nc4 *nc4p = (NC_nc4 *)ncdp;
...
int nc4io_sync_numrecs (void *ncdp) {
    /* Will NetCDF take care of this internally? */
    DEBUG_RETURN_ERROR (NC_ENOTSUPPORT)
}

int nc4io_sync (void *ncdp) {
    int err;
    NC_nc4 *nc4p = (NC_nc4 *)ncdp;
...
int nc4io_flush (void *ncdp) { DEBUG_RETURN_ERROR (NC_ENOTSUPPORT) }
